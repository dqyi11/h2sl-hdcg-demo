          ...
          for( l2 = l1->children; l2; l2 = l2->next ){
            if( l2->type == XML_ELEMENT_NODE ){
              if( xmlStrcmp( l2->name, ( const xmlChar* )( "feature_cv" ) ) == 0 ){
                ...
                _feature_groups.back().back()->from_xml( l2 );
              } else if ( xmlStrcmp( l2->name, ( const xmlChar* )( "feature_spatial_function_object_contains_child" ) ) == 0 ){
                _feature_groups.back().push_back( new Feature_Spatial_Function_Object_Contains_Child() );
                _feature_groups.back().back()->from_xml( l2 );
